...
RosPointCloudSplatfacto = MethodSpecification(
    config=ROSTrainerConfig(
        method_name="ros-pointcloud-splatfacto",
        steps_per_eval_image=100,
        steps_per_eval_batch=0,
        steps_per_save=2000,
        steps_per_eval_all_images=1000,
        ...
        pipeline=VanillaPipelineConfig(
            datamanager=ROSFullImageDataManagerConfig(
                _target=ROSFullImageDataManager[ROSPointCloudDataset],
                dataparser=ROSDataParserConfig(aabb_scale=1.0),
            ),
            model=ROSSplatfactoModelConfig(num_random=50),
        ),
    ...
    description="Run NerfBridge with the Splatfacto model, and train with streamed RGB images and point cloud data used to seed new gaussains.",
)
