...
class ROSPointCloudDataset(ROSDataset):
    """
    This is a tensor dataset that keeps track of all of the data streamed by ROS.
    It's main purpose is to conform to the already defined workflow of nerfstudio:
        (dataparser -> inputdataset -> dataloader).

    This variant uses point clouds instead of depth images to train the model.
    Point clouds are converted to depth maps during training.

    Images are tracked in self.image_tensor with uninitialized images set to
    all white (hence torch.ones). Point cloud data is stored in self.point_cloud_tensor.
    Poses are stored in self.cameras.camera_to_worlds as 3x4 transformation tensors.
    ...
    def __init__(
        ...
        super().__init__(dataparser_outputs, scale_factor, device)
        assert "point_cloud_topic" in dataparser_outputs.metadata.keys()

        self.point_cloud_topic_name = self.metadata["point_cloud_topic"]
        # Store point cloud data: variable length, so we use a list of tensors
        # Each element is a point cloud with shape (N, 3) for (x, y, z)
        self.point_cloud_tensor = [None] * self.num_images
    ...
    def __getitem__(self, idx: int):
        ...
        """
        data = super().__getitem__(idx)
        data["point_cloud"] = self.point_cloud_tensor[idx]
        return data
