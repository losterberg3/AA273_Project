...
from nerfbridge.ros_dataset import ROSDataset, ROSDepthDataset, ROSPointCloudDataset
...
class ROSDataloader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Flag for point cloud training, add point cloud tensor to data.
        self.listen_point_cloud = False
        if isinstance(self.dataset, ROSPointCloudDataset):
            self.listen_point_cloud = True
        ...
        if self.listen_point_cloud:
            self.subs.append(
                Subscriber(self.node, PointCloud2, self.dataset.point_cloud_topic_name)
            )
    ...
    def ts_callback(self, *args):
        ...
        if (
            ...
            # Process Point Cloud if using point cloud training
            if self.listen_point_cloud:
                pc_idx = 2 if self.listen_depth else 2
                self.point_cloud_callback(args[pc_idx])
    ...
    def image_callback(self, image: Image | CompressedImage):
        ...
        if im_tensor.ndim == 2:
            im_tensor = im_tensor.unsqueeze(-1).repeat(1, 1, 3)
    ...
    def point_cloud_callback(self, pointcloud: PointCloud2):
        """
        Callback for processing PointCloud2 messages. Extracts (x, y, z) coordinates
        from the point cloud and stores them as a tensor.
        """
        # Parse PointCloud2 message manually
        # Each point is represented as bytes in the data field
        ...
        # If we don't have x, y, z fields, store empty tensor
        if x_offset is None or y_offset is None or z_offset is None:
            points_tensor = torch.empty((0, 3), dtype=torch.float32)
            self.dataset.point_cloud_tensor[self.current_idx] = points_tensor
            return
        ...
        for i in range(width * height):
            ...
            # Unpack x, y, z as float32 (assuming standard float format)
            try:
                x = struct.unpack('f', data[x_offset + offset:x_offset + offset + 4])[0]
                y = struct.unpack('f', data[y_offset + offset:y_offset + offset + 4])[0]
                z = struct.unpack('f', data[z_offset + offset:z_offset + offset + 4])[0]
                
                # Skip NaN points
                if not (np.isnan(x) or np.isnan(y) or np.isnan(z)):
                    points_list.append([x, y, z])
            except (struct.error, IndexError):
                # Skip points that cannot be unpacked
                continue
        
        if len(points_list) > 0:
            # Convert to numpy array and apply scale
            points = np.array(points_list, dtype=np.float32)
            aggregate_scale = self.dataset.scene_scale_factor
            points_tensor = torch.from_numpy(points).to(dtype=torch.float32) * aggregate_scale
        else:
            # If no valid points, store empty tensor
            points_tensor = torch.empty((0, 3), dtype=torch.float32)
    ...
    def _get_updated_batch(self):
        ...
        # Handle point clouds separately since they are stored as list
        if self.listen_point_cloud:
            batch["point_cloud"] = self.dataset.point_cloud_tensor[: self.current_idx]
